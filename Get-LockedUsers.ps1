param (
    [string]$FilePath = "C:\LocalFolder", #create this folder in the PC
    [string]$DomainName = "domain.domain", #your domain
    [int]$CSVRetention = "90", #csv file retention
    $FileName = $null,
    $TempFileName = $null,
    [array]$NewValue = $null,
    [array]$NewFileData = $null,
    [string]$UserName = "*", #your username to search or * to search all users
    [datetime]$Today = (Get-Date -Format MM-dd-yy),
    [datetime]$StartTime = (Get-Date).AddDays(-3) #days range
)
